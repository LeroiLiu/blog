...
excerpt: 整理抖音 Web X-Bogus 参数的断点定位、补环境、算法分析、乱码生成和最终结果。
date: '2026-05-31T00:06:19'
categories:
  - 安全与逆向工程
tags:
  - 抖音
...
![流程分析示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_6QA86953ZBUW9MQ.jpg)
...
![msToken示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_PSZRW9MSRFDZ2HB.jpg)
...
![image-20240329104335637](/blog/images/security/douyin-x-bogus-reverse-notes/966701_43MPRK3SQY5NFPY.jpg)

然后跳到断点之后往上找断点。

到达如图所示位置之后

![xb示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_GX6A9QWC6C72TX8.jpg)
...
![image-20240329105633962](/blog/images/security/douyin-x-bogus-reverse-notes/966701_36JH5B7RNSK2SYW.jpg)

然后 单点调试下一步 ok 到达了一个JSVMP的方法。然后传的值都是字节码 然后把这些汇编代码转换为JS代码。

![xb示意图 2](/blog/images/security/douyin-x-bogus-reverse-notes/966701_NX9NVUWPNEEQ365.jpg)
...
![补环境示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_CF4BEXJECFSDB3A.jpg)
...
![补环境示意图 2](/blog/images/security/douyin-x-bogus-reverse-notes/966701_AD8SF9EEGQXEGZ2.jpg)

至于这个警告。是一个弃用警告。代表这个Buffer这个方法已经不用了。可以通过修改代码

然后看看传参是什么

![补环境示意图 3](/blog/images/security/douyin-x-bogus-reverse-notes/966701_6JUVNDQENMXM9HU.jpg)
...
![算法示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_NN9CM2BXR3Y2SZQ.jpg)

![算法示意图 2](/blog/images/security/douyin-x-bogus-reverse-notes/966701_FKRPK5MTTM25G65.jpg)

选择替换内容。浏览器会自动帮我们把js保存到替换的文件夹中。非常方便。

![算法示意图 3](/blog/images/security/douyin-x-bogus-reverse-notes/966701_3QYZYYQKCYUUJW3.jpg)

然后就能看到我们的文件了。然后复制文件。到v-tools中

![算法示意图 4](/blog/images/security/douyin-x-bogus-reverse-notes/966701_UNNBKFFEVF8S9NH.jpg)
...
![流程分析示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_5K4DS24HVCJZDVF.jpg)

![image-20240329140313200](/blog/images/security/douyin-x-bogus-reverse-notes/966701_R42JMU69RBG5WRG.jpg)

然后继续往上找栈。发现下图就是JSVMP初始化的地方。并且通过执行了W方法 最终获得返回值。

![image-20240329140613594](/blog/images/security/douyin-x-bogus-reverse-notes/966701_GXSSUKEBCJ3E5HG.jpg)
...
![流程分析示意图 2](/blog/images/security/douyin-x-bogus-reverse-notes/966701_G8RTZRT2C6RE78D.jpg)
...
![流程分析示意图 3](/blog/images/security/douyin-x-bogus-reverse-notes/966701_XK28HESKPDFPADK.jpg)
...
![流程分析示意图 4](/blog/images/security/douyin-x-bogus-reverse-notes/966701_VPJTKHV9BRCRZYB.jpg)

这里一定要注意debugger不要乱删 不然很容易卡死。打出来的数据非常之大。

如下图。把所有的日志断点保存 然后往上找第一个X-Bogus 生成的地方。

![流程分析示意图 5](/blog/images/security/douyin-x-bogus-reverse-notes/966701_CSSFZKVUSRVN5CM.jpg)

可以看到 如下图所示。是赋值 值给 X-Bogus的地方。那我们去搜索这个值

![流程分析示意图 6](/blog/images/security/douyin-x-bogus-reverse-notes/966701_V9354UVRCZJ9CQ9.jpg)

可以看到 这个地方应该就是初始值生成处

![流程分析示意图 7](/blog/images/security/douyin-x-bogus-reverse-notes/966701_4TEDHJCPF5FQN57.jpg)
...
![流程分析示意图 8](/blog/images/security/douyin-x-bogus-reverse-notes/966701_ZFQBXKS5YCGP8NC.jpg)
...
![image-20240329223848278](/blog/images/security/douyin-x-bogus-reverse-notes/966701_E426XBBWHVZRQ4U.jpg)

然后断住打断点继续往下走。

这里注意 要注意我们的O 里是否会有 如上值。如下图 才能正确单步调试。

![流程分析示意图 9](/blog/images/security/douyin-x-bogus-reverse-notes/966701_DQKMK38BFN5XSDM.jpg)

这里单步调试到下图位置。基本可以得到结论了

![流程分析示意图 10](/blog/images/security/douyin-x-bogus-reverse-notes/966701_KZEM98VZT2DP87K.jpg)
...
![流程分析示意图 11](/blog/images/security/douyin-x-bogus-reverse-notes/966701_YMQM5UR4CMS4ATG.jpg)

那这样基本其他值也可以用这个方法了。

![流程分析示意图 12](/blog/images/security/douyin-x-bogus-reverse-notes/966701_UDXF5EYWHK5UE4S.jpg)
...
![数字算法逻辑示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_HVUZZ7B4RWHVRCS.jpg)
...
![数字算法逻辑示意图 2](/blog/images/security/douyin-x-bogus-reverse-notes/966701_9E3Z5ZBCPQJBWFF.jpg)

      - 18通过 乱码charCodeAt得来

![image-20240330000330767](/blog/images/security/douyin-x-bogus-reverse-notes/966701_UW675YK2XRCXDCX.jpg)

    - 35072 通过137 << 8 左移的来

![image-20240329235922502](/blog/images/security/douyin-x-bogus-reverse-notes/966701_29KRTNYVH6T959B.jpg)

      - 137 通过乱码charCodeAt得来

![image-20240330000412236](/blog/images/security/douyin-x-bogus-reverse-notes/966701_TX44HC3Z2S3BGDY.jpg)
...
![数字算法逻辑示意图 3](/blog/images/security/douyin-x-bogus-reverse-notes/966701_D9MWGKF843RF3ZS.jpg)
...
![乱码生成分析示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_AVUHVFQPNNEKCN5.jpg)

![image-20240330003810920](/blog/images/security/douyin-x-bogus-reverse-notes/966701_EV4T4SA4CSN4YKR.jpg)

继续找索引去找这个第三个传参的生产处

![image-20240330003920354](/blog/images/security/douyin-x-bogus-reverse-notes/966701_8BC4R2XJ4TP7KXF.jpg)

如下图到达这个位置。再把这个函数扣下来。

![乱码生成分析示意图 2](/blog/images/security/douyin-x-bogus-reverse-notes/966701_3W6ABZG4A8ND6Z8.jpg)

如上图所示。这个b的生成还是没有还原出来。我们继续来找b。

下图 是生成b的地方。所以我们把这个函数也扣下来。

![乱码生成分析示意图 3](/blog/images/security/douyin-x-bogus-reverse-notes/966701_VS3KJGTPU947G2P.jpg)

我们测试下结果

![乱码生成分析示意图 4](/blog/images/security/douyin-x-bogus-reverse-notes/966701_2SJ53F4MB7R9V66.jpg)

可以正常生成。那我们再来测试下

![乱码生成分析示意图 5](/blog/images/security/douyin-x-bogus-reverse-notes/966701_8BATJVEATTQDTVG.jpg)
...
![最终结果示意图](/blog/images/security/douyin-x-bogus-reverse-notes/966701_XD3NS5DEWCS9ASG.jpg)
