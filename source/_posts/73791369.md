...
excerpt: 面象对向的三大特点：封装性、继承性、多态性 首先简单理解一下抽象： 我们在前面定义一个类的时候，实际上就是把一类事物共有的属性和行为提取出来，形成一个物理模型（模版），这种研究问题的方法称为抽象 封装性 封装就是把抽取出来的数据和对数据的操作封装在一起，数据被保护在内部，程序的其他部分只有被授权的操作（方法）才能对数据进行操作。 php提供了三种访问控制修饰
...
1. 如果子类要去调用父类的某个方法(`protected/public`)，可以使用 父类名::方法名 或者 parent::方法名
1. 在实现方法重写的时候，访问修饰符可以不一样，但是子类方法的访问权限必须大于等于父类方法的访问权限（即不能缩小父类方法的访问权限）
