...
excerpt: Traits 是一种为类似 PHP 的单继承语言而准备的代码复用机制。Trait 为了减少单继承语言的限制，使开发人员能够自由地在不同层次结构内独立的类中复用方法集 自 PHP 5.4.0 起，PHP 实现了代码复用的一个方法，称为 traits。 Traits 是一种为类似 PHP 的单继承语言而准备的代码复用机制。Trait 为了减少单继承语言的限制，使
...
上面就是些 Trait 比较基本的使用了，更详细的可以查看官方手册。这里总结下注意的几 点：
Trait 会覆盖调用类继承的父类方法
