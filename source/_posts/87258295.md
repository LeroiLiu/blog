...
excerpt: 针对PHP开发规范，有必要总结一下，与各位分享 用过IDE或看过其他源码的小伙伴们应该都见过类似下面这样的注释 看得多了就大概知道了一些规律。为了使自己的代码更加规zhuang范bi，也开始有样学样地写着这些注释。其实这种注释格式是有自己的名字的，它就叫—— PHPDOC 。 1、介绍 PHPDoc 是一个 PHP 版的 Javadoc。它是 一种注释 PH
...
> **PHPDoc** 是一个PHP版的Javadoc，也是一种注释PHP代码的正式标准。它支持通过类似phpDocumentor
...
>
> PHPDoc同时支持面向对象和面向过程的代码。

简单来说`PHPDOC`可以用来自动生成API文档。主流的IDE都会识别它，并在你coding中给予你相应的智能提示。使用PHPDOC有以下好处：
...
> 有关phpDoc的完整文档位于[phpDocumentor](https://www.phpdoc.org/docs/latest/index.html)官网。以下仅为整理的常用规范。
...
忽略
格式`@ignore [<描述>]`
例如你在if和else的语句块中定义分别同一个变量但值不同时，可以通过此标记让`phpDocumentor`忽略其中一个，以免生成重复的文档。例如
...
链接，可用于辅助说明、引用文档等
格式`@link [url] [<描述>]`
例如@link [https://www.google.com/](https://www.google.com/) 谷歌
...
**@property**
类属性，与`@method`类似，可以告诉IDE我这类里有哪些属性
