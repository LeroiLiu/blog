...
excerpt: 抽象类里的方法可以有方法体，只有抽象方法才不能拥有方法体。接口中的方法一定为抽象方法没有方法体，抽象类中的抽象方法才有方法体，但并不全为抽象方法。接口中虽然全部都为抽象方法，不可以定义属性，但是也可以定义常量。 可以使用abstract来修饰一个类或者方法。 用abstract修饰的类表示这个类是一个抽象类，用abstract修饰的方法表示这个方法是一个抽象
