...
excerpt: Swoole 安装、PHP 扩展、HTTP 服务、WebSocket 服务、协程、进程管理和常见问题。
...
Swoole 是 PHP 的高性能网络通信扩展，常用于常驻内存服务、HTTP 服务、WebSocket、`TCP/UDP` 服务、协程并发和异步任务。它和传统 PHP-FPM 的请求生命周期不同，写法上要特别注意全局状态和资源释放。
