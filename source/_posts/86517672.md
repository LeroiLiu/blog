...
excerpt: LNMP、WNMP、LAMP、WAMP 等开发环境安装，虚拟主机创建，以及 Laravel 和 Dingo 开发环境配置记录。
...
Linux 平台可以使用 [LNMP一键安装包](https://lnmp.org/)：

LNMP一键安装包是一个用Linux Shell编写的可以为`CentOS/RHEL/Fedora/Aliyun/Amazon`、`Debian/Ubuntu/Raspbian/Deepin/Mint` Linux VPS或独立主机安装`LNMP(Nginx/MySQL/PHP)、LNMPA(Nginx/MySQL/PHP/Apache)、LAMP(Apache/MySQL/PHP)`生产环境的Shell程序。
...
登陆后运行：`screen -S lnmp`
如果提示screen: command not found 命令不存在可以执行：`yum install screen` 或 `apt-get install screen`安装，详细内容可以查看[screen教程](https://www.vpser.net/manage/run-screen-lnmp.html)。
...
**接下来，将以`Linux(CentOS7)`+Nginx+MySQL+PHP7的开发环境进行…**
...
**当然，如果你想直接使用公司提供的开发包，可以直接clone本仓库到根目录下，然后**
...
添加一些基础文件，便于后期操作。Dingo/Api 的更多说明可以查看 [Dingo/Api](https://github.com/dingo/api/wiki) 维基。
